Sample_theta<-function(z, X, pii, A, sigma_0, prob, alpha, sigma_1)
{

###################################################################

## USAGE
 # Sample_theta(z, X, pii, A, sigma_0, prob, alpha, sigma_1)
...
## Densities
    ...
    if(L>1)
    {
         for (i in 1:L)
         {
             f1z<-f1z+prob[i]*dnorm(z, mean=X%*%matrix(alpha[i, ],q,1), sd=sigma_1[i])
         }
    }else{
    	f1z<-f1z+prob*dnorm(z, mean=X%*%matrix(alpha, ncol=1), sd=sigma_1)
    }
